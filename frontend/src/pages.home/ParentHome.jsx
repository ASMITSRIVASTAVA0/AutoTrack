...
const ParentHome = () => {
  ...
  const [isHoveringStats, setIsHoveringStats] = useState({});
  ...
  const [addingSafetyZone, setAddingSafetyZone] = useState(false);
  const [newZone, setNewZone] = useState({ name: '', radius: 500, location: null });
  const [showScheduleRide, setShowScheduleRide] = useState(false);
  const [scheduledRides, setScheduledRides] = useState([]);
  const [newScheduledRide, setNewScheduledRide] = useState({
    childId: '',
    pickup: '',
    destination: '',
    scheduledTime: '',
  ...
  useEffect(() => {
    ...
    const handleEmergencyAlert = (data) => {
      ...
      addNotification(`🚨 EMERGENCY ALERT from ${data.childName}`, 'error', true);
    ...
    socket.on('parent-request-accepted-notification', (data) => {
      addNotification(`${data.userName} accepted your parent request!`, 'success');
      loadParentData();
    });
    ...
    return () => {
      ...
      socket.off('parent-request-accepted-notification');
    };
  }, [parent, socket, selectedChild]);
  ...
  // All other functions remain the same (loadParentData, sendChildRequest, etc.)
  // ... [Previous functions remain unchanged, just update styling]
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-gray-900 via-black to-purple-900 relative overflow-hidden'>
      ...
      <div className='relative z-10 max-w-7xl mx-auto p-6'>
        {/* Header with Emergency Button - Animated */}
          ...
          <div className='flex gap-3'>
            {emergencyMode && (
              <button
                ...
                className='relative px-6 py-3 bg-gradient-to-r from-rose-600 to-rose-700 text-white rounded-xl hover:from-rose-700 hover:to-rose-800 transition-all duration-300 font-semibold flex items-center gap-2 shadow-2xl shadow-rose-500/30 hover:shadow-rose-500/50 animate-pulse hover:scale-105 active:scale-95'
              >
                <div className='absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full hover:translate-x-full transition-transform duration-1000'></div>
                <i className="ri-alarm-warning-line animate-bounce"></i>
                Emergency Active
              </button>
            )}
            <button
              // onClick={triggerEmergency}
              className='relative px-6 py-3 bg-gradient-to-r from-rose-600 to-rose-700 text-white rounded-xl hover:from-rose-700 hover:to-rose-800 transition-all duration-300 font-semibold flex items-center gap-2 shadow-2xl shadow-rose-500/30 hover:shadow-rose-500/50 hover:scale-105 active:scale-95 group'
            >
              <div className='absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000'></div>
              <i className="ri-emergency-light-line group-hover:rotate-90 transition-transform duration-500"></i>
              Emergency
            </button>
            <Link 
              to='/parent/logout' 
        ...
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
          ...
          <div className='lg:col-span-2 space-y-6'>
            ...
            <div className='relative group animate-slideInUp' style={{ animationDelay: '800ms' }}>
              ...
              <div className='relative bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-xl rounded-2xl p-6 border border-white/10'>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button
                    ...
                    className="relative px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg hover:from-purple-700 hover:to-pink-700 transition-all duration-300 font-semibold flex items-center gap-2 shadow-2xl hover:shadow-purple-500/25 hover:scale-105 active:scale-95 group/btn"
                  >
                    <div className='absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover/btn:translate-x-full transition-transform duration-1000'></div>
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    ...
                                    <button
                                      onClick={() => {
                                        setNewScheduledRide(prev => ({...prev, childId: child._id}));
                                        setShowScheduleRide(true);
                                      }}
                                      className="px-3 py-1 bg-purple-500/20 text-purple-400 rounded-lg text-sm hover:bg-purple-500/30 transition-colors hover:scale-105"
                                    >
                                      Schedule Ride
                                    </button>
                                    <button
            ...
            {/* Safety Zones Management - Animated */}
            <div className='relative group animate-slideInUp' style={{ animationDelay: '900ms' }}>
              <div className='absolute -inset-1 bg-gradient-to-r from-fuchsia-600 to-violet-600 rounded-2xl blur-xl opacity-0 group-hover:opacity-30 transition-opacity duration-500'></div>
              <div className='relative bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-xl rounded-2xl p-6 border border-white/10'>
                <div className="flex justify-between items-center mb-4">
                  <h2 className='text-xl font-semibold text-white flex items-center gap-2'>
                    <div className='relative w-8 h-8 bg-gradient-to-br from-fuchsia-500 to-violet-500 rounded-lg flex items-center justify-center shadow-lg'>
                      <i className="ri-shield-check-line text-white"></i>
                    </div>
                    Safety Zones ({safetyZones.length})
                  </h2>
                  <button
                    onClick={() => setAddingSafetyZone(true)}
                    className="relative px-4 py-2 bg-gradient-to-r from-fuchsia-600 to-violet-600 text-white rounded-lg hover:from-fuchsia-700 hover:to-violet-700 transition-all duration-300 font-semibold flex items-center gap-2 shadow-2xl hover:shadow-fuchsia-500/25 hover:scale-105 active:scale-95"
                  >
                ...
                {safetyZones.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-white/50 text-lg font-medium">No safety zones configured</p>
                    <p className="text-white/30 text-sm mt-1">Create safety zones to get alerts when children enter/leave</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {safetyZones.map((zone, index) => (
                      <div
                        key={zone._id}
                        className="group/item relative p-4 bg-gradient-to-r from-white/5 to-transparent rounded-xl border border-white/10 hover:border-fuchsia-500/50 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-fuchsia-500/10"
                        style={{ animationDelay: `${index * 100}ms` }}
                      >
                        <div className="absolute inset-0 bg-gradient-to-r from-fuchsia-500/10 to-violet-500/10 rounded-xl opacity-0 group-hover/item:opacity-100 transition-opacity duration-300"></div>
                        <div className="relative flex justify-between items-center">
                          <div className="flex-1">
                            <p className="font-semibold text-white flex items-center gap-2">
                              <i className="ri-map-pin-line text-fuchsia-400 group-hover/item:animate-bounce"></i>
                              {zone.name}
                            </p>
                            <p className="text-sm text-white/60 mt-1">
                              Radius: {zone.radius}m • Children: {zone.children?.length || 0}
                            </p>
                          </div>
                          <button
                            onClick={() => removeSafetyZone(zone._id)}
                            className="px-3 py-1 bg-rose-500/20 text-rose-400 rounded-lg hover:bg-rose-500/30 transition-colors text-sm hover:scale-105"
                          >
                            Remove
                          </button>
                        </div>
          ...
          <div className='space-y-6'>
            ...
            {selectedChild ? (
              <div className='relative group animate-slideInUp' style={{ animationDelay: '1000ms' }}>
            ...
            <div className='relative group animate-slideInUp' style={{ animationDelay: '1100ms' }}>
            ...
            {/* Scheduled Rides - Animated */}
            {scheduledRides.length > 0 && (
              <div className='relative group animate-slideInUp' style={{ animationDelay: '1200ms' }}>
                <div className='absolute -inset-1 bg-gradient-to-r from-purple-600 to-violet-600 rounded-2xl blur-xl opacity-0 group-hover:opacity-30 transition-opacity duration-500'></div>
                <div className='relative bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-xl rounded-2xl p-6 border border-white/10'>
                  <h3 className="font-semibold text-white mb-4 flex items-center gap-2">
                    <div className='relative w-7 h-7 bg-gradient-to-br from-purple-500 to-violet-500 rounded-lg flex items-center justify-center shadow-lg'>
                      <i className="ri-calendar-todo-line text-white"></i>
                    </div>
                    Scheduled Rides ({scheduledRides.length})
                  </h3>
                  <div className="space-y-3 max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-pink-500/30 scrollbar-track-transparent">
                    {scheduledRides.slice(0, 3).map(ride => (
                      <div key={ride._id} className="group/item p-3 bg-gradient-to-r from-white/5 to-transparent rounded-lg border border-white/10 hover:border-purple-500/50 transition-all duration-300 hover:scale-[1.02]">
                        <div className="flex justify-between items-start">
                          <div>
                          ...
                          <button
                            ...
                            className="text-rose-500/60 hover:text-rose-400 transition-colors hover:scale-110"
                          >
                            <i className="ri-close-line"></i>
                          </button>
                        </div>
